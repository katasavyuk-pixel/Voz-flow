...
import { Mic, Square, Zap, Copy, RefreshCw, Check, Sparkles } from "lucide-react";
...
export default function VoiceRecorder() {
    ...
    }, [isRecording]); // Necesitamos el estado actual para saber si empezar o parar
    ...
    const startRecording = async () => {
        try {
            ...
            toast.error("No se pudo acceder al micrófono. Por favor, revisa tus permisos.");
        }
    };
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-center gap-1.5 h-6">
                                {[1, 2, 3, 4, 5, 4, 3, 2, 1].map((h, i) => (
                                    <div key={i} className="w-1 rounded-full bg-purple-400 animate-pulse" style={{ height: `${h * 20}%`, animationDelay: `${i * 0.1}s` }} />
                                ))}
                            </div>
                            <p className="text-sm font-bold text-gray-500 tracking-widest uppercase">Capturando Audio Pro...</p>
                        </div>
                    ) : isProcessing ? (
                        <div className="flex flex-col items-center gap-3">
                            <RefreshCw className="w-8 h-8 text-purple-400 animate-spin" />
                            <p className="text-sm font-bold text-purple-400 uppercase tracking-widest">Groq Whisper Engine Procesando</p>
                        </div>
                    ) : refinedText ? (
                        <div className="space-y-4 w-full">
                            ...
                            <div className="flex items-center justify-center gap-2">
                                <span className="text-xs font-bold text-gray-500 flex items-center gap-1">
                                    <Sparkles className="w-3 h-3 text-purple-400" /> COPIADO AL PORTAPAPELES
                                </span>
                            </div>
                        </div>
                    ) : (
                        <p className="text-lg text-gray-500 font-medium italic">
                            Haz clic para grabar. Usaremos IA profesional para un resultado perfecto.
                        </p>
                    )}
                </div>
